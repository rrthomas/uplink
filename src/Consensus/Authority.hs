...
data BlockValidationError
  ...
  | BlockGenLimitSurpassed Int Address    -- ^ When block is generated by a validator within the blockGenLimit
  | BlockSignerLimitSurpassed Address     -- ^ When block contains a signature from a node that has signed a block too recently
  ...
  | ValidatorAccountDoesNotExist Address  -- ^ Block signer address does not refer to an existing account in the ledger
