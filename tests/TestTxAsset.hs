...
txAssetTest :: Int -> QC.Property
txAssetTest k = monadicIO $ do
    ...
    let acc1PrivKey = snd keys1
        ...
        holder1     = Asset.Holder addr1
        holder2     = Asset.Holder addr2
    ...
    (world2, errs2, _ ) <- mkTransferTx addr1 holder2 25000 world1
    assert $ null errs2
    ...
    (world4, errs4, _) <- mkTransferTx addr2 holder1 25001 world3
    assert $ not $ null errs4

    -- Assert holdings are correct:
    let correctHoldings = Asset.Holdings $ Map.fromList [(holder1, 25000), (holder2, 25000)]
...
txAssetTests =
  testGroup "TxAsset tests"
    [ QC.testProperty "TxAsset" txAssetTest
    ]
